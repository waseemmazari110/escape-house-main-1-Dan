...
const MAX_SUBMISSIONS_PER_IP = 1; // Only 1 submission per day
const MIN_FORM_FILL_TIME_MS = 12000; // 12 seconds minimum
const MAX_FORM_FILL_TIME_MS = 15 * 60 * 1000; // 15 minutes maximum
const MIN_USER_CLICKS = 3;
const MIN_USER_KEYSTROKES_TRUSTED = 10;
const MIN_USER_KEYSTROKES_UNTRUSTED = 15;
...
function validateChallenge(challenge: string): boolean {
  try {
    const expectedValue = Math.floor(Date.now() / 10000).toString();
    return challenge === expectedValue;
  } catch {
    return false;
  }
}
